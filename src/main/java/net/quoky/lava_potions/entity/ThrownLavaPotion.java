...
public class ThrownLavaPotion extends ThrowableItemProjectile {
    ...
    public ThrownLavaPotion(Level level, LivingEntity entity) {
        ...
        this.setDeltaMovement(
                -Math.sin(entity.getYRot() * ((float) Math.PI / 180F))
                        * Math.cos(entity.getXRot() * ((float) Math.PI / 180F)) * velocityFactor,
                -Math.sin(entity.getXRot() * ((float) Math.PI / 180F)) * velocityFactor - 0.1F,
                Math.cos(entity.getYRot() * ((float) Math.PI / 180F))
                        * Math.cos(entity.getXRot() * ((float) Math.PI / 180F)) * velocityFactor);
    }
    ...
    protected void onHit(HitResult hitResult) {
        ...
        if (!this.level().isClientSide) {
            ...
            // Use the potion directly - no need for getRegularVariant since we only have
            // base types
            ...
            // Check if this is a lingering potion based on the item stack NBT or custom
            // logic
            boolean isLingering = isLingeringPotion(itemstack);

            // Apply immediate fire effect to all living entities in radius for both splash
            ...
            // Apply impact damage only for basic lava potions (lava bottle and awkward
            // lava)
            if (ModPotionTypes.isBasicLavaPotion(basePotion)) {
    ...
    protected void onHitEntity(EntityHitResult hitResult) {
        ...
        if (target instanceof LivingEntity livingEntity) {
            ...
            if (ModPotionTypes.isBasicLavaPotion(basePotion)) {
                ...
                // Apply direct hit damage of 6 hit points (3 hearts) but don't damage fire
                // immune entities
    ...
    private void createAreaEffectCloud(ItemStack itemStack, Potion potion, Potion basePotion) {
        AreaEffectCloud cloud = new AreaEffectCloud(this.level(), this.getX(), this.getY(), this.getZ()) {
            // Override to set entities on fire and apply damage when they're in the
            // lingering cloud
            @Override
            public void tick() {
                super.tick();
                ...
                if (this.tickCount % 10 == 0) {
                    ...
                    for (LivingEntity entity : entitiesInCloud) {
                        ...
                        if (ModPotionTypes.isBasicLavaPotion(basePotion)) {
                            ...
                            if (basePotion == ModPotionTypes.OBSIDIAN_SKIN.get()
                                    || basePotion == ModPotionTypes.OBSIDIAN_SKIN_LONG.get()) {
                                // No lingering effects for obsidian skin
                            }
                            // Netherite Skin: No lingering damage, no fire
                            else if (basePotion == ModPotionTypes.NETHERITE_SKIN.get()
                                    || basePotion == ModPotionTypes.NETHERITE_SKIN_LONG.get()) {
                                // No lingering effects for netherite skin
                            }
                            // Glass Skin: No lingering damage, no fire
                            else if (basePotion == ModPotionTypes.GLASS_SKIN.get()
                                    || basePotion == ModPotionTypes.GLASS_SKIN_LONG.get()) {
                                // No lingering effects for glass skin
                            }
                            // Heat: Sets entities on fire for 10 seconds
                            else if (basePotion == ModPotionTypes.HEAT.get()
                                    || basePotion == ModPotionTypes.HEAT_LONG.get() ||
                                    basePotion == ModPotionTypes.HEAT_STRONG.get()) {
                                entity.setSecondsOnFire(10);
                            }
                            // Flammability: No lingering damage, no fire
                            else if (basePotion == ModPotionTypes.FLAMMABILITY.get()
                                    || basePotion == ModPotionTypes.FLAMMABILITY_LONG.get()) {
                                // No lingering effects for flammability
                            }
                            // Pyromancy: Sets entities on fire for 15 seconds
                            else if (basePotion == ModPotionTypes.PYROMANCY.get()
                                    || basePotion == ModPotionTypes.PYROMANCY_LONG.get() ||
                                    basePotion == ModPotionTypes.PYROMANCY_STRONG.get()) {
                                entity.setSecondsOnFire(15);
                            }
                            // Magma Walker: Sets entities on fire for 5 seconds
                            else if (basePotion == ModPotionTypes.MAGMA_WALKER.get()
                                    || basePotion == ModPotionTypes.MAGMA_WALKER_LONG.get() ||
        ...
        if (ModPotionTypes.isBasicLavaPotion(basePotion)) {
            ...
            // Lava Vision
            else if (basePotion == ModPotionTypes.LAVA_VISION.get()
    ...
    public void handleEntityEvent(byte id) {
        if (id == 3) {
            ...
            if (ModPotionTypes.isBasicLavaPotion(basePotion)) {
                // Basic potions: explosion particle + burst of flame particles + ring of lava
                // particles
                ...
                if (basePotion == ModPotionTypes.OBSIDIAN_SKIN.get()
                    ...
                    burstB = 0.0f; // 0/255
                        ...
                        || basePotion == ModPotionTypes.LAVA_VISION_LONG.get()) {
                    // Green color for lava vision (0x00ca98)
                    burstR = 0.0f; // 0/255
                    burstG = 0.792f; // 202/255
                    burstB = 0.596f; // 152/255
                } else if (basePotion == ModPotionTypes.LAVA_STRIDER.get()
                        || basePotion == ModPotionTypes.LAVA_STRIDER_LONG.get()
                        || basePotion == ModPotionTypes.LAVA_STRIDER_STRONG.get()) {
                    // Blue color for lava strider (0x005ff4)
                    burstR = 0.0f; // 0/255
                    burstG = 0.373f; // 95/255
                    burstB = 0.957f; // 244/255
                }
                ...
                for (int i = 0; i < 36; i++) {
                    ...
                    if (basePotion == ModPotionTypes.OBSIDIAN_SKIN.get()
                        ...
                        // Green color for lava vision (0x00ca98)
                        ringR = 0.0f; // 0/255
                        ringG = 0.792f; // 202/255
                        ringB = 0.596f; // 152/255
                    } else if (basePotion == ModPotionTypes.LAVA_STRIDER.get()
                            || basePotion == ModPotionTypes.LAVA_STRIDER_LONG.get()
                            || basePotion == ModPotionTypes.LAVA_STRIDER_STRONG.get()) {
                        // Blue color for lava strider (0x005ff4)
