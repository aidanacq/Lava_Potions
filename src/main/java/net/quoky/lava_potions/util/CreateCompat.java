...
public class CreateCompat {
    ...
    static {
        ...
        if (ALEXSMOBS_LOADED) {
            Lava_Potions.LOGGER
                    .info("AlexsMobs detected - using priority mixins and recipes to override lava bottle conflicts");
        }
    }
     ...
     * This can be used by other parts of the mod to interact with Create's fluid
     * system
     */
    public static FluidStack getLavaPotionFluid(Potion potion, int amount) {
        ...
        try {
            ...
            // Special case for awkward lava potion - use Create's potion fluid with our
            // custom metadata
                ...
                try {
                    ...
                    if (result != null && !result.isEmpty()) {
                        Lava_Potions.LOGGER.debug(
                                "Created custom awkward lava fluid with ID: create:potion/lava_potions/awkward_lava");
                        return result;
                    }
                } catch (Exception e) {
            ...
            Class<?> bottleTypeClass = Class
                    .forName("com.simibubi.create.content.fluids.potion.PotionFluid$BottleType");

            Object regularBottleType = Enum.valueOf((Class<Enum>) bottleTypeClass, "REGULAR");

            java.lang.reflect.Method ofMethod = potionFluidClass.getMethod("of", int.class, Potion.class,
                    bottleTypeClass);
            ...
            if (result != null && !result.isEmpty() && ModPotionTypes.isLavaPotion(potion)) {
                if (potion == ModPotionTypes.AWKWARD_LAVA.get() || ModPotionTypes.isBaseLavaBottle(potion)) {
                    ...
                    result.getOrCreateTag().putString("LavaFlowingTextureOverride",
                            "lava_potions:block/gray_lava_flow");
                }
            }
